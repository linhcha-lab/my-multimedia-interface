...
export default function StudentSAEList() {
  ...
  const navigate = useNavigate() // ✅ corrigé
    ...
    <PageLayoutStudent title="Liste SAE">
      ...
      <div>
        {filtrees.map((sae) => (
          <div
            key={sae.id}
            onClick={() => navigate(`/student/saes/${sae.id}`)} // ✅ navigation corrigée
            style={{
              background: "#fff",
              padding: 16,
              borderRadius: 12,
              marginBottom: 10,
              cursor: "pointer", // UX améliorée
            }}
          >
            ...
            <div>Progression : {sae.progression}%</div>
          </div>
        ))}
      </div>
    </PageLayoutStudent>
