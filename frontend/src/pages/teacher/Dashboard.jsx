    ...
    <PageLayout title="Tableau de bord">
      <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 14 : 20 }}>
        ...
        <div style={{ display: "grid", gap: 16, gridTemplateColumns: isMobile ? "1fr" : isTablet ? "1fr 1fr" : "1fr 1.6fr 1fr" }}>
          ...
          {/* Planifier */}
          <div onClick={() => setShowModal(true)} style={{ background: "#f4f0fb", borderRadius: 18, padding: 22, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, cursor: "pointer", minHeight: 155, transition: "box-shadow .2s, transform .15s" }}
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: 20, boxShadow: "0 2px 18px rgba(0,0,0,.06)" }}>
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 17, fontWeight: 800, color: "#1a1a2e", marginBottom: 12 }}>Messages</h2>
            {MESSAGES.map(m => (
              <MessageRow key={m.id} name={m.name} unread={m.unread}
        ...
        <div style={{ display: "grid", gap: 16, gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr" }}>
          <div style={{ background: "#ede9fb", borderRadius: 18, padding: 20 }}>
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 17, fontWeight: 800, color: "#1a1a2e", marginBottom: 14 }}>Avancée des groupes</h2>
            {GROUP_PROGRESS.map((g, i) => <GroupCard key={i} {...g} />)}
          </div>
          <div style={{ background: "#fff", borderRadius: 18, padding: 20, boxShadow: "0 2px 18px rgba(0,0,0,.06)" }}>
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 17, fontWeight: 800, color: "#1a1a2e", marginBottom: 14 }}>Statistiques rapides</h2>
            {QUICK_STATS.map(s => (
            ...
            {isMobile && <div style={{ display: "flex", justifyContent: "center", marginTop: 22 }}><CircularProgress value={70} size={130} /></div>}
          </div>
        </div>
      </div>
