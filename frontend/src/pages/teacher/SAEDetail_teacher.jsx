...
// Fix : le state des dépôts est dans SAEDetail (parent)
// → les validations persistent quand on change d'onglet
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(26,26,46,.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, backdropFilter:"blur(4px)", padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:22, padding:32, width:"100%", maxWidth:520, boxShadow:"0 30px 80px rgba(0,0,0,.2)", maxHeight:"85vh", overflowY:"auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:20, fontWeight:800, color:"#1a1a2e" }}>{groupe.nom} — Détail</h2>
          <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", color:"#aaa", fontSize:24, lineHeight:1 }}>×</button>
        </div>
        ...
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {groupe.taches.map(t => (
              <div key={t.id} style={{ display:"flex", alignItems:"center", gap:12, background:t.done?"#f0fdf4":"#f8f6fc", borderRadius:10, padding:"11px 14px", border:`1px solid ${t.done?"#86efac":"#f0edf8"}` }}>
                <div style={{ width:22, height:22, borderRadius:6, flexShrink:0, background:t.done?"#c8f000":"#fff", border:t.done?"none":"2px solid #d4c5f5", display:"flex", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
                  {t.done && (
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", marginTop:24 }}>
          <button onClick={onClose} style={{ padding:"10px 24px", borderRadius:12, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>Fermer</button>
        </div>
      </div>
    </div>
  )
...
// ─────────────────────────────────────────────────────────────
// Onglets — reçoivent tous leurs données en props depuis SAEDetail
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)", marginBottom:16 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:14 }}>Progression globale</div>
      ...
      <div style={{ background:"#f8f6fc", borderRadius:16, padding:22, marginBottom:16, border:"1px solid #f0edf8" }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:14 }}>Informations clés</div>
        {[
...
function TabGroupes({ detail }) {
  ...
  return (
    <div>
      <div style={{ fontFamily:"'Syne',sans-serif", fontSize:18, fontWeight:800, color:"#1a1a2e", marginBottom:20 }}>Groupes participants</div>
            ...
            <div key={g.id} style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)", border:g.retard?"2px solid #fca5a5":"1px solid #f0edf8" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <div style={{ width:42, height:42, borderRadius:12, background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:16 }}>{g.id}</div>
                  <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                  ...
                  <button onClick={() => setGroupeDetail(g)} style={{ padding:"7px 16px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:6 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {detail.ressources.map(r => (
          <div key={r.id} style={{ background:"#fff", borderRadius:14, padding:"14px 18px", display:"flex", alignItems:"center", gap:14, boxShadow:"0 2px 10px rgba(0,0,0,.05)" }}>
            ...
            <button style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #e0daf5", background:"none", fontSize:12, color:"#7c3aed", fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Télécharger</button>
          </div>
        ))}
      </div>
    </div>
...
// ── TabDepots — lecture seule, pas de validation ─────────────
function TabDepots({ detail }) {
  // Pour chaque groupe, on calcule si le dépôt est en retard
  // (date du dépôt > date limite de la SAE)
  const dateLimite = new Date(detail.dateLimite)
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        {detail.groupes.map(g => {
          // Dépôts de ce groupe
          const depotsDuGroupe = detail.depots.filter(d => d.groupe === g.nom)
          const aDepose = depotsDuGroupe.length > 0

          return (
            <div key={g.id} style={{
              background:"#fff", borderRadius:14, padding:20,
              boxShadow:"0 2px 10px rgba(0,0,0,.05)",
              border: !aDepose ? "2px solid #fca5a5" : "1px solid #f0edf8",
            }}>
              {/* En-tête groupe */}
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: aDepose ? 14 : 0, flexWrap:"wrap", gap:8 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:800, fontSize:14 }}>
                    {g.id}
                  </div>
                  <div>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:15, color:"#1a1a2e" }}>{g.nom}</div>
                    <div style={{ fontSize:11, color:"#aaa" }}>{g.etudiants.join(", ")}</div>
                  </div>
                </div>

                {/* Badge statut global du groupe */}
                {!aDepose
                  ? <span style={{ background:"#fce7e7", color:"#991b1b", padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700 }}>⚠ Aucun dépôt</span>
                  : <span style={{ background:"#dcfce7", color:"#166534", padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700 }}>✓ {depotsDuGroupe.length} dépôt{depotsDuGroupe.length > 1 ? "s" : ""}</span>
                }
              </div>

              {/* Liste des dépôts du groupe */}
              {aDepose && (
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {depotsDuGroupe.map(d => {
                    const dateDepot = new Date(d.date)
                    const enRetard  = dateDepot > dateLimite
                    return (
                      <div key={d.id} style={{
                        display:"flex", alignItems:"center", gap:12,
                        background: enRetard ? "#fff7f7" : "#f8f6fc",
                        borderRadius:10, padding:"10px 14px",
                        border:`1px solid ${enRetard ? "#fca5a5" : "#f0edf8"}`,
                      }}>
                        <div style={{ width:32, height:32, borderRadius:8, background:"#f0ebff", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                          <Svg d={IC.file} size={15} color="#7c3aed" />
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:13, fontWeight:600, color:"#1a1a2e" }}>{d.etudiant}</div>
                          <div style={{ fontSize:11, color:"#aaa" }}>{d.fichier} · déposé le {jj(d.date)}</div>
                        </div>
                        {enRetard && (
                          <span style={{ background:"#fce7e7", color:"#991b1b", padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:700, flexShrink:0 }}>
                            ⚠ En retard
                          </span>
                        )}
                      </div>
...
export default function SAEDetail() {
  ...
  // ✅ State des dépôts supprimé — TabDepots lit directement depuis detail
    ...
    <PageLayout title="">
      <div style={{ background:"#7c3aed", borderRadius:16, overflow:"hidden", marginBottom:0 }}>
        ...
        <div style={{ padding:"10px 24px 0", display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,.4)", marginBottom:4, textTransform:"uppercase", letterSpacing:.5 }}>{sae.semestre}</div>
