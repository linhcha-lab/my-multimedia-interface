// ============================================================
//  pages/teacher/SAEDetail.jsx
//  La progression des tâches est en LECTURE SEULE pour le prof.
//  C'est l'étudiant qui coche dans SAEDetailStudent.jsx
// ============================================================
    ...
    groupes: [
      {
        ...
        rendusEffectues: 2, rendusAttendus: 3, retard: false,
        taches: [
      ...
      },
      {
        ...
        rendusEffectues: 1, rendusAttendus: 3, retard: true,
        taches: [
          { id:1, label:"Analyse des besoins",       done:true  },
      ...
      },
      {
        ...
        rendusEffectues: 3, rendusAttendus: 3, retard: false,
        taches: [
          { id:1, label:"Analyse des besoins",       done:true },
          { id:2, label:"Maquettes Figma",            done:true },
          { id:3, label:"Configurer le projet React", done:true },
...
const jj = (d) => new Date(d).toLocaleDateString("fr-FR")

function EtatBadge({ statut }) {
  const c = { "En cours":{ bg:"#e0f2fe",color:"#0369a1" }, "Terminé":{ bg:"#dcfce7",color:"#166534" }, "À venir":{ bg:"#fef9c3",color:"#854d0e" } }[statut] || { bg:"#f0edf8",color:"#7c3aed" }
  return <span style={{ background:c.bg, color:c.color, padding:"4px 12px", borderRadius:20, fontSize:12, fontWeight:700 }}>{statut}</span>
}

function StatBox({ label, value, color="#7c3aed" }) {
  return (
    <div style={{ background:"#f8f6fc", borderRadius:14, padding:"16px 20px", flex:1, minWidth:120, border:"1px solid #f0edf8" }}>
      <div style={{ fontSize:11, color:"#999", marginBottom:6, textTransform:"uppercase", letterSpacing:.4 }}>{label}</div>
      <div style={{ fontFamily:"'Payton Oneyne',sans-serif", fontSize:24, fontWeight:800, color }}>{value}</div>
    </div>
  )
}
...
function ProgressBar({ value, color="#7c3aed", height=8 }) {
  return (
    <div style={{ background:"#f0edf8", borderRadius:4, height, overflow:"hidden" }}>
      <div style={{ height:"100%", borderRadius:4, width:`${value}%`, background: value===100 ? "#22c55e" : `linear-gradient(90deg,${color},#9d5cf5)`, transition:"width .5s ease" }} />
    </div>
  )
}

// ── Modale groupe — LECTURE SEULE pour le prof ────────────────
function GroupeDetailModal({ groupe, onClose }) {
  const done  = groupe.taches.filter(t => t.done).length
  const total = groupe.taches.length
  const prog  = Math.round((done / total) * 100)

  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(26,26,46,.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, backdropFilter:"blur(4px)", padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:22, padding:32, width:"100%", maxWidth:520, boxShadow:"0 30px 80px rgba(0,0,0,.2)", maxHeight:"85vh", overflowY:"auto" }}>

        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          <h2 style={{ fontFamily:"'Payton One',sans-serif", fontSize:20, fontWeight:800, color:"#1a1a2e" }}>{groupe.nom} — Détail</h2>
          <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", color:"#aaa", fontSize:22 }}>×</button>
        </div>
        ...
        <div style={{ background:"#f8f6fc", borderRadius:12, padding:14, marginBottom:20 }}>
          ...
          {groupe.etudiants.map(e => (
            <div key={e} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
              <div style={{ width:28, height:28, borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:11 }}>{e.charAt(0)}</div>
        ...
        {/* ── Tâches en LECTURE SEULE — le prof ne peut pas cocher ── */}
        <div>
          <div style={{ fontSize:11, fontWeight:700, color:"#7c3aed", marginBottom:6, textTransform:"uppercase", letterSpacing:.4 }}>Avancement des tâches</div>
          {/* Note explicite pour le prof */}
          <p style={{ fontSize:11, color:"#aaa", fontStyle:"italic", marginBottom:12 }}>
            Les tâches sont cochées par les étudiants — la progression se met à jour automatiquement.
          </p>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {groupe.taches.map(t => (
              <div key={t.id} style={{
                ...
                border:`1px solid ${t.done ? "#86efac" : "#f0edf8"}`,
                // Pas de cursor:pointer, pas d'onClick → lecture seule
              }}>
                {/* Checkbox visuelle non cliquable */}
                <div style={{
                ...
                <span style={{ fontSize:13, color:"#2d2d4e", textDecoration:t.done?"line-through":"none", opacity:t.done?.6:1, flex:1 }}>
                  {t.label}
                </span>
                <span style={{ fontSize:11, fontWeight:600, color:t.done?"#16a34a":"#bbb" }}>
                  {t.done ? "✓ Terminé" : "En attente"}
                </span>
              </div>
            ))}
          </div>
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", marginTop:24 }}>
          <button onClick={onClose} style={{ padding:"10px 24px", borderRadius:12, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>Fermer</button>
        </div>
      </div>
    </div>
  )
...
function TabInfos({ sae, detail }) {
  ...
  const totalEtudiants = detail.groupes.reduce((a,g) => a+g.etudiants.length, 0)
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)", marginBottom:16 }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:14 }}>Progression globale</div>
        ...
        <div style={{ fontSize:11, color:"#aaa", marginTop:6 }}>Du {jj(detail.dateDebut)} au {jj(detail.dateLimite)}</div>
      </div>

      <div style={{ background:"#f8f6fc", borderRadius:16, padding:22, marginBottom:16, border:"1px solid #f0edf8" }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:14 }}>Informations clés</div>
        {[
          ...
          { label:"Étudiants",     value:<span style={{ fontSize:13, color:"#555" }}>{totalEtudiants} étudiants</span> },
        ].map(({ label, value }) => (
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)", marginBottom:16 }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:10 }}>Description</div>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)" }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:15, fontWeight:800, color:"#1a1a2e", marginBottom:10 }}>Consignes</div>
...
function TabGroupes({ detail }) {
  ...
  return (
    <div>
      <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:18, fontWeight:800, color:"#1a1a2e", marginBottom:20 }}>Groupes participants</div>
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        {detail.groupes.map(g => {
          const done = g.taches.filter(t => t.done).length
          const prog = Math.round((done/g.taches.length)*100)
          return (
            <div key={g.id} style={{ background:"#fff", borderRadius:16, padding:22, boxShadow:"0 2px 16px rgba(0,0,0,.06)", border:g.retard?"2px solid #fca5a5":"1px solid #f0edf8" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <div style={{ width:42, height:42, borderRadius:12, background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontFamily:"'Payton One',sans-serif", fontWeight:800, fontSize:16 }}>{g.id}</div>
                  <div>
                    <div style={{ fontFamily:"'Payton One',sans-serif", fontWeight:800, fontSize:16, color:"#1a1a2e" }}>{g.nom}</div>
                    <div style={{ fontSize:12, color:"#aaa" }}>{g.etudiants.length} étudiant{g.etudiants.length>1?"s":""}</div>
                  </div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  {g.retard && <span style={{ background:"#fce7e7", color:"#991b1b", padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700 }}>⚠ En retard</span>}
                  <span style={{ background:g.rendusEffectues===g.rendusAttendus?"#dcfce7":"#fef9c3", color:g.rendusEffectues===g.rendusAttendus?"#166534":"#854d0e", padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700 }}>
                    {g.rendusEffectues}/{g.rendusAttendus} rendus
                  </span>
                  <button onClick={() => setGroupeDetail(g)} style={{ padding:"7px 16px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"Plus Jakarta Sans", display:"flex", alignItems:"center", gap:6 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:14 }}>
                {g.etudiants.map(e => (
                  <div key={e} style={{ display:"flex", alignItems:"center", gap:7, background:"#f8f6fc", borderRadius:20, padding:"5px 12px", fontSize:12, color:"#2d2d4e" }}>
                    <div style={{ width:22, height:22, borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:9 }}>{e.charAt(0)}</div>
                    {e}
                  </div>
                ))}
              </div>
      ...
      {groupeDetail && <GroupeDetailModal groupe={groupeDetail} onClose={() => setGroupeDetail(null)} />}
    </div>
  )
}
...
function TabRessources({ detail }) {
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:18, fontWeight:800, color:"#1a1a2e" }}>Ressources</div>
        <button style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>
          <Svg d={IC.plus} size={14} color="#fff" /> Ajouter
        </button>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {detail.ressources.map(r => (
          <div key={r.id} style={{ background:"#fff", borderRadius:14, padding:"14px 18px", display:"flex", alignItems:"center", gap:14, boxShadow:"0 2px 10px rgba(0,0,0,.05)" }}>
            ...
            <button style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #e0daf5", background:"none", fontSize:12, color:"#7c3aed", fontWeight:600, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>Télécharger</button>
          </div>
        ))}
      </div>
    </div>
...
function TabDepots({ detail }) {
  ...
  const StatutBadge = ({ statut }) => {
    const c = { en_attente:{ label:"En attente",bg:"#fef9c3",color:"#854d0e" }, valide:{ label:"Validé",bg:"#dcfce7",color:"#166534" }, refuse:{ label:"Refusé",bg:"#fce7e7",color:"#991b1b" } }[statut]
    return <span style={{ background:c.bg, color:c.color, padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:600 }}>{c.label}</span>
  }
  return (
    <div>
      <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:18, fontWeight:800, color:"#1a1a2e", marginBottom:20 }}>Dépôts</div>
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {items.map(d => (
          <div key={d.id} style={{ background:"#fff", borderRadius:14, padding:"14px 18px", display:"flex", alignItems:"center", gap:14, boxShadow:"0 2px 10px rgba(0,0,0,.05)", flexWrap:"wrap" }}>
            ...
            {d.statut === "en_attente" && (
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={() => setItems(p => p.map(v => v.id===d.id?{...v,statut:"refuse"}:v))} style={{ padding:"6px 12px", borderRadius:8, border:"1.5px solid #fca5a5", background:"none", fontSize:11, color:"#ef4444", fontWeight:600, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>Refuser</button>
                <button onClick={() => setItems(p => p.map(v => v.id===d.id?{...v,statut:"valide"}:v))} style={{ padding:"6px 12px", borderRadius:8, border:"none", background:"#22c55e", fontSize:11, color:"#fff", fontWeight:600, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>Valider</button>
              </div>
            )}
          </div>
        ))}
...
function TabAnnonces({ detail }) {
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <div style={{ fontFamily:"'Payton One',sans-serif", fontSize:18, fontWeight:800, color:"#1a1a2e" }}>Annonces</div>
        <button style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#7c3aed,#9d5cf5)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"Plus Jakarta Sans" }}>
          <Svg d={IC.plus} size={14} color="#fff" /> Nouvelle annonce
        </button>
      </div>
...
export default function SAEDetail() {
    ...
    return (
      <PageLayout title="SAE introuvable">
        <div style={{ textAlign:"center", padding:"60px 20px" }}>
          ...
          <button onClick={() => navigate("/teacher/saes")} style={{ marginTop:16, padding:"10px 20px", borderRadius:10, border:"none", background:"#7c3aed", color:"#fff", cursor:"pointer", fontFamily:"'Plus Jakarta Sans'" }}>Retour à la liste</button>
        </div>
      </PageLayout>
    )
  }
  ...
  return (
    <PageLayout title="">
      <div style={{ background:"#7c3aed", borderRadius:16, overflow:"hidden", marginBottom:0 }}>
        <button onClick={() => navigate("/teacher/saes")} style={{ display:"flex", alignItems:"center", gap:6, background:"none", border:"none", cursor:"pointer", color:"rgba(255,255,255,.4)", fontSize:12, padding:"14px 20px 0", fontFamily:"'Plus Jakarta Sans'" }}>
        ...
        <div style={{ padding:"10px 24px 0", display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            ...
            <h1 style={{ fontFamily:"'Paytone One',sans-serif", fontSize:24, fontWeight:900, color:"#c8f000", lineHeight:1.2 }}>{sae.code} — {sae.titre}</h1>
          </div>
          <EtatBadge statut={detail.statut} />
        </div>
        <div style={{ display:"flex", padding:"0 20px", overflowX:"auto", marginTop:14 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ padding:"13px 20px", background:"none", border:"none", borderBottom: tab===t.id?"2px solid #1a1a2e":"2px solid transparent", color: tab===t.id?"#fff":"rgba(255,255,255,.45)", fontWeight: tab===t.id?700:400, fontSize:13, cursor:"pointer", fontFamily:"'Plus Jakarta Sans'", whiteSpace:"nowrap", transition:"all .15s" }}>
              {t.label}
            </button>
          ))}
        </div>
