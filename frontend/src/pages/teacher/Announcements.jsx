...
import { ANNONCES } from "../../data/mockData";
...
export default function Announcements() {
  ...
  const handleCreate = () => {
    ...
    const nouvelle = { id: Date.now(), ...form, date: new Date().toISOString().split("T")[0] };
          ...
          <div key={a.id} style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,.06)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
              <div>
                ...
                <span style={{ fontSize: 11, color: "#bbb" }}>{new Date(a.date).toLocaleDateString("fr-FR")}</span>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button style={{ background: "none", border: "none", cursor: "pointer", color: "#9d5cf5" }}>
                  <Svg d={IC.edit} size={16} />
                </button>
                <button onClick={() => setAnnonces(prev => prev.filter(x => x.id !== a.id))}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "#f87171" }}>
                  <Svg d={IC.trash} size={16} />
                </button>
              </div>
            </div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#1a1a2e", marginBottom: 8 }}>{a.titre}</div>
            <div style={{ fontSize: 13, color: "#666", lineHeight: 1.6 }}>{a.contenu}</div>
          </div>
        ))}
      </div>
      ...
      {showModal && (
        <Modal title="Nouvelle annonce" onClose={() => setShowModal(false)}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
            ...
            <button onClick={handleCreate} style={{ flex: 1, padding: 11, borderRadius: 12, border: "none", background: "linear-gradient(135deg,#7c3aed,#9d5cf5)", fontSize: 14, cursor: "pointer", color: "#fff", fontWeight: 700 }}>Publier</button>
          </div>
        </Modal>
      )}
    </PageLayout>
